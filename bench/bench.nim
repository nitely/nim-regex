...
func genText(): string {.compileTime.} =
  ...
  #result.add("ฅ")
#const text = genText()
const text = "650-253-0001"

var pattern2 = re.re"^[0-9]+-[0-9]+-[0-9]+$"
...
const pattern4 = regex.re2(r"(?-u)^[0-9]+-[0-9]+-[0-9]+$") #, {regex.RegexFlag.reAscii})
var env = regex.newRegexEnv(10)

benchRelative(regex_sol, m):
  var m2: regex.RegexMatch2
  for i in 0 ..< m:
    discard regex.match(text, pattern4, m2, env)
  doNotOptimizeAway(m2)

benchRelative(regex_macro_sol, m):
  var d: bool
  for i in 0 ..< m:
    regex.match text, regex.rex"(?-u)^[0-9]+-[0-9]+-[0-9]+$", env:
      d = true
  doNotOptimizeAway(d)
#
...
proc benfmt(name: string, timeIter: int, itersSec: float) =
  echo &"{name:<20} | {timeIter}ns time/iter | {itersSec:.2f} iters/s"

template benchmark2(name: string, runs: int, body: untyped) =
  let dstart = getMonoTime()
  for _ in 0 ..< runs:
    body
  let dend = getMonoTime() - dstart
  let totalSec = dend.inNanoseconds().float / 1_000_000_000.0
  benfmt(name, dend.inNanoseconds() div runs, runs.float / totalSec)

proc benchy() =
  var m2: regex.RegexMatch2
  var count = 0
  var env2 = regex.newRegexEnv(10)
  benchmark2("regex_sol", 20000):
    regex.match text, regex.rex"(?-u)^[0-9]+-[0-9]+-[0-9]+$", env2:
      count += 1
    #count += int(regex.match(text, pattern4, m2, env2))
  echo count
...
$ vtune -collect hotspots ./bin/bench
$ vtune -collect hotspots -knob sampling-mode=hw -knob sampling-interval=0.5 ./bin/bench
$ vtune -collect memory-consumption ./bin/bench
$ perf record -F 999 -g -- ./test2
$ nim c --debugger:native --threads:off -d:danger -d:useMalloc --passC:"-g -fno-inline -no-pie" --passL:"-g -no-pie" -o:bin/bench bench/bench.nim
