...
func splitIncl*(s: string, sep: Regex2, limit = -1, start = 0): seq[string] {.raises: [].} =
  ...
  while not done:
    ...
    for mi in ms:
      ...
      last = ab.a
      if ab.a > start or ab.a <= ab.b:  # skip first empty match
...
when isMainModule:
  ...
  proc toString(pattern: Regex2): string =
    ## NFA to string representation.
