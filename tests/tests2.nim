...
test "tsplitIncl":
  ...
  check splitIncl("abc", re2"", limit = -1) == @["a", "b", "c"]
  check splitIncl("abc", re2"", limit = 0) == @[]
  check splitIncl("abc", re2"", limit = 1) == @["abc"]
  check splitIncl("abc", re2"", limit = 2) == @["a", "bc"]
  check splitIncl("a,b,c", re2"(,)", limit = -1) == @["a", ",", "b", ",", "c"]
  check splitIncl("a,b,c", re2"(,)", limit = 0) == @[]
  check splitIncl("a,b,c", re2"(,)", limit = 1) == @["a,b,c"]
  check splitIncl("a,b,c", re2"(,)", limit = 2) == @["a", ",", "b,c"]
  check splitIncl("a1b2c", re2"(\d)", limit = -1) == @["a", "1", "b", "2", "c"]
